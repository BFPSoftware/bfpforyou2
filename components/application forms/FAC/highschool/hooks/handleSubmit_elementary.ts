import sendConfirmationEmail from '@/hooks/confirmation/sendConfirmationEmail';
import { ElementaryType } from '../schema/elementary';
...
const createAddRecord = (formResponse: ElementaryType) => {
    return {
        ...
        Attachment1: { value: [{ fileKey: formResponse.attachment1 }] },
        Attachment2: { value: [{ fileKey: formResponse.attachment2 }] },
        Attachment3: { value: [{ fileKey: formResponse.attachment3 }] },
        ...
        Spouse_Birthday: {
            value: `${convertMonthShortToNumber(formResponse.spouse.spouseBirthday?.month || '')}/${formResponse.spouse.spouseBirthday?.day}/${
                ...
                formResponse.children.childStatus == 'No'
                    ...
                    : formResponse.children.childTable &&
                      formResponse.children.childTable.map((child) => {
                          if (!child) return;
                          const childBirthday = `${child.childBirthday?.year}-${convertMonthShortToNumber(child.childBirthday?.month || '')}-${
                              child.childBirthday?.day
                          }`;
                          return {
                              value: {
...
export const handleSubmit_newImmigrant = async (formResponse: ElementaryType, t: any) => {
    try {
