...
export const buildElementaryProfileFields = (record: FacKintoneRecord): void => {
    ...
    record.profile_family = {
        value: [`I live with my ${whoDoYouLiveWith}`, `We speak ${languageAtHome} at home`, `What I love about my family is that ${aboutFamily}`].join("\n"),
    };

    record.profile_school = {
        value: [`My favorite subject in school is ${favoriteSubject}.`, `I struggle with ${challengingSubject}.`, `My teacher says that ${aboutMeFromTeacher}`].join("\n"),
    };

    record.profile_aboutMe = {
        value: [`My favorite color is ${favoriteColor}.`, `My favorite food is ${favoriteFood},`, `I love to ${hobbies}.`, `The thing that makes me sad is ${makesMeSad}.`, `What I love most in the world is ${loveMost}.`, `When I grow up I want to become ${futureDreams}.`].join("\n"),
    };
};

export const isElementaryApplication = (record: FacKintoneRecord): boolean => record.applicationType?.value === "Elementary";
